...
int main(int argc, char* argv[]) {
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0] << " [-dxbc | -dxil | -spirv] <input> [output.hlsl]\n";
        return 1;
    }
    ...
    if (mode == "-dxbc" && !fs::exists(dxbc2dxil)) missingTools.push_back("dxbc2dxil.exe");
    if ((mode == "-dxbc" || mode == "-dxil") && !fs::exists(dxilSpirv)) missingTools.push_back("dxil-spirv.exe");
    if (!fs::exists(spirvCross)) missingTools.push_back("spirv-cross.exe");
